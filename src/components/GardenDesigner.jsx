...
const GardenDesigner = ({ beds, setBeds, designName, orientation, scale, setScale, bgColor, setBgColor }) => {
  ...
  // Generera HTML för design (används både för print och PDF)
    ...
    if (printWindow) {
      printWindow.document.write(`
        ...
        <html>
          <head>
            <meta charset="UTF-8">
            <title>Trädgårdsdesign - ${escapeHtml(designName)}</title>
          </head>
          <body>
            ${generateDesignHTML()}
          </body>
  ...
  const exportAsPDF = () => {
    ...
    element.innerHTML = generateDesignHTML();
                ...
                return (
                  <React.Fragment key={bed.id}>
                    ...
                    {bed.showText !== false && (() => {
                      ...
                      // För Konva Text: när width och height är satta, så är x/y vänsterkant/överkant
                      // align/verticalAlign centrerar texten inom width/height
                      // Vi sätter width/height till stora värden så att texten kan vara större än bädden
                      // men fortfarande centreras korrekt
                      const textWidth = 10000; // Stort värde så att texten inte begränsas
                      const textHeight = 10000; // Stort värde så att texten inte begränsas
